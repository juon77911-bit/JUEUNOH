...
const ProjectPage = () => {
      ...
      if (trimmedLine.startsWith('*')) {
        return (
          <div key={i} className="flex gap-4 items-start mb-4">
            <span className="mt-2.5 w-2 h-2 bg-main shrink-0" />
            <span 
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-main selection:text-black font-sans relative overflow-hidden select-none">
      {/* Background Effects */}
      ...
      <div 
        ...
        style={{
          background: `radial-gradient(150px circle at ${mousePos.x}px ${mousePos.y}px, ${data.config.accentColor}14, transparent 100%)`
        }}
      />

      {/* Progress Bar */}
      <motion.div
        className="fixed top-0 left-0 right-0 h-1 bg-main z-[110] origin-left"
        style={{ scaleX }}
      />

      {/* Navigation Header */}
      <header className="fixed top-0 left-0 right-0 z-[100] bg-[#1A1A1A]/95 backdrop-blur-xl border-b border-main/30 px-6 py-10">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-8 relative z-10">
          ...
          <div className="flex flex-wrap gap-8 md:gap-16 pb-1">
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-bold uppercase tracking-widest text-main">Date</span>
              <span className="text-sm font-medium text-[#DEDEDE]">{project.date}</span>
            </div>
            {project.tools && project.tools.length > 0 && (
              <div className="flex flex-col gap-1">
                <span className="text-[10px] font-bold uppercase tracking-widest text-main">Tools</span>
                  ...
                  <div className="flex flex-col gap-12">
                    {block.title && (
                      <div className="flex items-center gap-6">
                        <div className="w-12 h-[3px] bg-main" />
      ...
      <footer className="px-6 py-32 border-t border-white/5 relative z-20">
        <div className="max-w-5xl mx-auto">
          <span className="text-[10px] font-bold uppercase tracking-[0.5em] text-main mb-12 block text-center">Up Next</span>
          
          <Link 
            to={`/project/${nextProject.id}`}
            className="group block relative p-12 md:p-20 rounded-[40px] border border-white/10 bg-zinc-900/20 overflow-hidden transition-all duration-700 hover:border-main/40 hover:bg-zinc-900/40"
          >
            {/* Hover Glow */}
            <div className="absolute inset-0 bg-main/[0.03] opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
            <div className="absolute -inset-[100%] bg-gradient-to-r from-transparent via-main/10 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
            
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
              <div className="flex flex-col gap-6 text-center md:text-left">
                <h3 
                  className="font-black tracking-tighter leading-none group-hover:text-main transition-colors duration-500"
              ...
              <div 
                className="w-24 h-24 rounded-full border border-white/20 flex items-center justify-center group-hover:border-main group-hover:bg-main group-hover:text-black transition-all duration-700 shrink-0 shadow-[0_0_20px_rgba(0,0,0,0)]"
                style={{ 
                  boxShadow: '0 0 0px var(--main-color)' 
                }}
              >
